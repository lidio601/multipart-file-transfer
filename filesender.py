...
class FileIOProtocol(basic.LineReceiver):
    ...
    def lineReceived(self, line):
        ...
        #print self.factory.options
        file_path = self.factory.options['output_dir']
    ...
    def connectionLost(self, reason):
        ...
        # Success uploading - tmpfile will be saved to disk.
            ...
            print self.instruction.items()
            self.factory.db.update({'crc':self.crc,'file_size':self.size,'client':client,'new_file':self.outfilename,'original_file':self.original_fname,'file_metadata':fileinfo(self.outfilename),'upload_time':datetime.datetime.now()})
            self.factory.db.update(self.instruction)
...
class myServerFactory(policies.ThrottlingFactory):
	#protocol.Factory):
	def __init__(self):
		self.wrappedFactory = ServerFactory() #protocol.Factory()
	...
	def buildProtocol(self, addr):
		...
		return FileIOProtocol(self)

class FileIOFactory(myServerFactory): #ServerFactory):
...
class FileIOClient(basic.LineReceiver):
    ...
    def connectionMade(self):
        ...
        instruction.update(self.controller.instruction)
